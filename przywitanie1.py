...
"gridline-color: rgb(170, 0, 0);")
        ...
        nowalista = []
        i = 0
        for el in listaużytkownika:
            liczba = ''
            if (listaużytkownika[i].isdigit()):

                liczba += (el)
                if (listaużytkownika[i + 1].isdigit()):
                    liczba += (listaużytkownika[i + 1])
        ...
        def usun():
            ...
            for data2 in zaznaczone:
                ...
                for i in tyt:
                    tytul+=i
                #print("zaznaczony tytuł: ",tytul)
                #print("zaznaczony autor: ",autor)
...
class Ui_Youbook(object):
    def setupUi(self, Youbook):
        ...
        def dodajliste():
            ...
            # wynikf = con2.execute('SELECT * FROM FORMULARZE WHERE USERNAME=?',użytkownik)
            # print(wynikf)
...
class Ui_Youbook2(object):
    ...
    def retranslateUi(self, Youbook2):
        ...
        self.label_5.setText(_translate("Youbook2", "2.Jeżeli chcesz zobaczyć listę swoich książek wejdz w ikone 'Moja Lista' na ekranie lub w menu \n "
        ...
        self.label_2.setText(_translate("Youbook2", "4.Aby zaznaczyć że jakieś książki juz przeczytałeś/aś zaznacz pozycję na liście i kliknij 'Zapisz zmiany'"))
